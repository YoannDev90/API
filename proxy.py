...
async def proxy_request(request, path: str):
    ...
    logger.info(f"Proxy: {request.method} {request.url.path} -> {target_url} from {client_ip}")
    ...
    try:
        ...
        return Response('{"code": "504", "message": "Gateway Timeout"}', status_code=504, media_type="application/json")
    except httpx.ConnectError:
        logger.error(f"Connection error proxying to {target_url}")
        return Response('{"code": "502", "message": "Bad Gateway"}', status_code=502, media_type="application/json")
    except Exception as e:
        logger.error(f"Proxy error for {target_url}: {e}")
        return Response('{"code": "500", "message": "Internal Server Error"}', status_code=500, media_type="application/json")
